...
export class SurvivalGame extends Phaser.Scene {
    ...
    constructor() {
        ...
        this.survivalTime = 0; // Tempo sobrevivido
        this.isGameRunning = false; // Flag que controla se o jogo está ativo
    ...
    create() {
        ...
        // Resetar estados
        this.survivalTime = 0; // Zera o tempo
        this.isGameRunning = true; // Marca que o jogo está rodando
    ...
    createUI() {
        this.uiBg = this.add
            .rectangle(0, 0, 150, 75, 0x000000, 0.4)
            .setOrigin(0, 0)
            .setDepth(1000)
        ...
        this.moneyText = this.add
            .text(0, 0, ``, { ...textStyle, color: "#ffff00" })
            .setOrigin(0, 0)
            .setDepth(1000);
    ...
    updateUIPosition() {
        ...
        const barWidth = 150 / zoom,
            barHeight = 20 / zoom;
        this.healthBarBg
            .setPosition(offsetX + 39 / zoom, offsetY + 210 / zoom)
            .setSize(barWidth + 2 / zoom, barHeight + 2 / zoom);
        this.healthBar
            .setPosition(offsetX + 40 / zoom, offsetY + 211 / zoom)
            .setSize(barWidth * (this.playerHp / this.playerMaxHp), barHeight);
    }
    ...
    createUpgradeAreas() {
        ...
        const areaData = [
            { point: perkPointForca, cost: 2000, upgrade: "forca" },
            { point: perkPointReviver, cost: 1500, upgrade: "reviver" },
            { point: perkPointResistencia, cost: 2500, upgrade: "resistencia" },
            { point: perkPointRecarga, cost: 3000, upgrade: "recarga" },
        ];

        areaData.forEach((data) => {
            if (!data.point) {
                console.error(
                    `ERRO: Objeto de perk '${data.upgrade}' não encontrado.`
                );
                return;
            }
            const area = this.add
    ...
    createWeaponAreas() {
        ...
        const weaponDefinitions = [
            ...
            { tiledName: "doze", internalName: "shotgun", cost: 2500 },
            { tiledName: "rifre", internalName: "rifle", cost: 2000 },
        ];
        ...
        weaponDefinitions.forEach((def) => {
            ...
            if (!weaponPoint) {
                console.error(
                    `ERRO: Objeto de arma '${def.tiledName}' não encontrado.`
                );
                return;
            }
            const area = this.add
            ...
            if (this.currentUpgradeArea) {
                ...
                if (area.upgradeType === "reviver" && this.revivedOnce) {
                    this.showTemporaryMessage(
                        "Ressurgimento só pode ser usado uma vez.",
                        "error"
                    );
                ...
                if (this.money >= area.cost) {
                    ...
                    const iconKey =
                        area.upgradeType === "recarga"
                            ? "velocidade"
                            : area.upgradeType;
                    this.addPerkIcon(iconKey);
                    this.showTemporaryMessage(
                        `Upgrade de ${area.upgradeName} comprado!`
                    );
                } else {
    ...
    showTemporaryMessage(text, soundType = "default") {
        ...
        this.upgradeText
            .setText(text)
            .setPosition(this.player.x - 90, this.player.y - 50)
            .setVisible(true);
    ...
    gameOver() {
        if (this.canRevive && !this.revivedOnce) {
            ...
            const reviveIconIndex = this.perkIcons.findIndex(
                (icon) => icon.texture.key === "perk_reviver"
            );
    ...
    update(time, delta) {
        ...
        if (this.isGameRunning) {
            this.survivalTime += delta / 1000; // Adiciona em segundos
        }
    ...
    updateUI() {
        const remainingZombies = Math.max(
            0,
            this.zombiesToSpawnForRound - this.zombiesKilledThisRound
        );
        this.roundText.setText(
            `Round: ${this.round} | Faltam: ${remainingZombies}`
        );
        this.scoreText.setText(`Pontos: ${this.score}`);
        this.moneyText.setText(`Ouro: ${this.money}`);

        const weapon1 = this.weapons[0],
            weapon2 = this.weapons[1];
        const capitalize = (s) =>
            s ? s.charAt(0).toUpperCase() + s.slice(1) : "Empty";
        ...
        this.weaponSlot1Bg.setFillStyle(
            this.currentWeaponIndex === 0 ? 0x00ff00 : 0x333333
        );
        this.weaponSlot2Bg.setFillStyle(
            this.currentWeaponIndex === 1 ? 0x00ff00 : 0x333333
        );
    }
    ...
    moveZombiesTowardsPlayer() {
        this.zombies.children.iterate((zombie) => {
            ...
            this.simulateDirectionalAnimation(
                zombie,
                zombie.type,
    ...
    spawnZombie() {
        ...
        Object.assign(zombie, {
            ...
            type,
            frameToggleTime: 0,
            frameToggleState: false,
            isTakingDamage: false,
            hitFrameToggle: false,
            originalWidth: zombie.displayWidth,
            originalHeight: zombie.displayHeight,
        });
    ...
    spawnBossZombie() {
        ...
        const boss = this.physics.add
            .sprite(x, y, "boss_down")
            .setDisplaySize(48, 48)
            .setOrigin(0.5)
            .setDepth(5);
        Object.assign(boss, {
            hp: this.zombieBaseHp * 10,
            speed: this.zombieBaseSpeed * 0.8,
            type: "boss",
            frameToggleTime: 0,
            frameToggleState: false,
            isTakingDamage: false,
            hitFrameToggle: false,
            originalWidth: boss.displayWidth,
            originalHeight: boss.displayHeight,
        });
        ...
        this.zombies.children.iterate((zombie) => {
            ...
            if (now > data.lastCheckTime + CHECK_INTERVAL) {
                const distanceMoved = Phaser.Math.Distance.Between(
                    zombie.x,
                    zombie.y,
                    data.positionAtLastCheck.x,
                    data.positionAtLastCheck.y
                );
                data.stuckTime =
                    distanceMoved < MINIMUM_PROGRESS_DISTANCE
                        ? data.stuckTime + CHECK_INTERVAL
                        : 0;
                    ...
                    this.time.delayedCall(
                        ...
                        () => {
                            if (zombie && zombie.active) {
                                ...
                                zombie.setAlpha(1.0);
                                data.phasingCooldown =
                                    this.time.now + PHASING_COOLDOWN;
                            }
                        },
                        [],
                        this
    ...
    spawnBloodEffect(x, y) {
        ...
        this.time.addEvent({
            ...
            callback: () => blood.setTexture(frames[++frameIndex]),
        });
    ...
    simulateDirectionalAnimation(zombie, baseKey, vx, vy) {
        ...
        if (now > zombie.frameToggleTime) {
            ...
            zombie
                .setTexture(`${baseKey}_${direction}${frame}`)
                .setDisplaySize(zombie.originalWidth, zombie.originalHeight);
    ...
    startNewRound() {
        ...
        this.roundZombieSpawner = this.time.addEvent({
            ...
            callback: () => {
                if (
                    this.zombiesSpawnedThisRound < this.zombiesToSpawnForRound
                ) {
                    this.spawnZombie();
                    this.zombiesSpawnedThisRound++;
                } else {
                    if (this.roundZombieSpawner)
                        this.roundZombieSpawner.destroy();
    ...
    setupCollisions() {
        ...
        const zombieWallCollisionProcess = (zombie, wallObject) => {
            ...
            if (wallObject.properties && wallObject.properties.colisao_p)
                return false;
            return true;
        };
        ...
        this.physics.add.collider(
            ...
            (bullet, tile) => !tile.properties.colisao_p,
            this
        );
    }
